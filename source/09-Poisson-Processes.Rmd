...
library(tidyverse)
treshold = 0.4
points = tibble(x=c(0,ts),y=c(0,cumsum(has)), is_black=FALSE, is_white = FALSE)
...
ggplot(lines, aes(x=x, y=y, xend=xend, yend=yend, linetype=is_jump)) +
  geom_segment()+
  scale_linetype_manual(name = "Linetype", values=c(1, 0))+
  geom_point(data = subset(lines, is_black), aes(x=x,y=y), size=1)+
  geom_point(data = subset(lines, is_whiteend), aes(x=xend,y=yend), shape=21, size=1, fill="white")+
  theme_minimal()+
...
\to 0$. This limit will exist, however, only if we let $p \to 0$ at the same
time. To see why, we take $t = n \Delta t$ and observe that 
$N_t$ has the binomial distribution 
with parameters $n$ and $p$, and, consequently $\EE[ N_t] = np$. 
Letting $\Delta t \to 0$ while keeping $p$ and $t$ fixed amounts to letting $n\to \infty$ and, with it, the expected value of $\EE[ N_t]$. The way to keep this expectation from exploding is to simulataneously decrease keep it from
